	...
	<view class="container">
		...
		<block v-if="(!!!searchValue)">
			<view class="citylist-content">
				<view :data-title="section.title" class="section" v-for="(section,index) in (citylist)"
					:key="section.title">
					<view class="section-title">{{section.title}}</view>
					<view :class="section.style==='inline'?'section-body-inline':'section-body'">
						<view class="section-item" @tap="selectCity" :data-city="city"
							v-for="(city,index1) in (section.items)" :key="city.id">{{city.city_name}}</view>
					</view>
				</view>
			</view>
			<!-- 右侧导航栏 -->
			<view class="citylist-nav" id="test" @touchmove.stop.prevent="handleTouchmove"
				@touchstart="handleTouchstart" @touchend="handleTouchend">
				<view class="citylist-nav-item" @tap="navSelect" :data-index="index" v-for="(item,index) in (citylist)"
					:key="item.title">{{item.style==='inline'?item.index:item.title}}</view>
			</view>
		</block>
		<!-- 搜索条件不为空时显示搜索结果列表 -->
		<block v-else>
			<view class="result-box">
				<block v-if="result.length">
					<view :data-city="city" @tap="selectCity" class="result-item" v-for="(city,index) in (result)"
						:key="city.id">
						{{city.city_name}}
					</view>
				</block>
			...
			search(e) {
				...
				if (value) {
					result = citys.filter(item => {
						...
						return item.city_name.includes(value) || item.city_pinyin.toUpperCase().includes(value) ||
							item.city_short
...
<style>
	...
	.section-body .section-item:last-child {
		border: none;
	}
	...
	.section-body-inline .section-item {
		min-width: 26%;
		background: #fff;
		margin-top: 30rpx;
		margin-right: 4%;
